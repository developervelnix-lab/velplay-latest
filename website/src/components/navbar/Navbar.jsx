...
function Navbar() {
  const { accountInfo, showLogin, setShowLogin, showRegister, setShowRegister, refreshSiteData, activateDemoMode , navbarCategories, unreadCount, markAllNotificationsRead } = useSite();
  ...
  const games = ALL_GAMES.filter(g => navbarCategories && navbarCategories.includes(g.name));
              ...
              <div className="hidden md:flex items-center gap-2 ml-2 flex-none">
                ...
                >
                  ...
                  <button
                    ...
                    className="w-9 h-9 rounded-full border border-white/5 bg-[#151b29] hover:bg-brand/15 hover:border-brand/30 text-white/80 hover:text-white transition-all duration-300 flex items-center justify-center shadow-[0_6px_16px_rgba(0,0,0,0.25)] relative"
                    title="Notifications"
                  ...
                  {notifOpen && (
                    <div className="absolute right-0 top-full mt-3 w-80 md:w-96 rounded-3xl border border-white/10 bg-[#10131d]/95 backdrop-blur-2xl shadow-[0_20px_60px_rgba(0,0,0,0.45)] overflow-hidden z-[260]">
                      <div className="p-4 border-b border-white/5 flex items-center justify-between">
                        <div className="flex items-center gap-2">
                          <FaBell className="text-brand text-xs" />
                          <span className="text-xs font-black uppercase text-white tracking-wider">Notifications</span>
                        </div>
                        <button
                          onClick={() => {
                            setNotifOpen(false);
                        ...
                        {(() => {
                          ...
                          const replaceTags = (txt) => {
                            ...
                            return txt
                              ...
                              .replace(/@userid/gi, u.account_id || userId || '')
                              .replace(/@id/gi, u.account_id || userId || '')
                              .replace(/@uid/gi, u.account_id || userId || '')
                          ...
                          return list.slice(0, 6).map((n, i) => (
                            <div key={n.id || i} className="p-3 rounded-2xl bg-white/[0.03] border border-white/5 hover:border-brand/20 transition-all">
                              <div className="flex items-center justify-between mb-1">
                                <span className="text-[11px] font-black text-white uppercase tracking-wide truncate max-w-[180px]">
                                  {replaceTags(n.title)}
                                </span>
                                <span className="text-[8px] font-bold px-1.5 py-0.5 rounded bg-brand/15 text-brand uppercase tracking-wider">
                                  Notice
                                </span>
                              </div>
                      ...
                      <div className="p-3 border-t border-white/5 bg-black/20">
                        <button
                          ...
                          className="w-full py-2.5 rounded-2xl bg-brand text-black font-black uppercase text-[10px] tracking-wider transition-all hover:brightness-110 flex items-center justify-center gap-1.5 shadow-lg"
                        >
