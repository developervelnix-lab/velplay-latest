...
import { FaDownload, FaShieldAlt, FaChevronRight } from 'react-icons/fa';
...
const RanaFooter = () => {
  ...
  const { accountInfo, openAppInstall, isAppReady } = useSite();
  ...
  const logoUrl = accountInfo?.service_site_logo?.startsWith('http')
    ...
    : (!accountInfo?.service_site_logo ? '/image.png' : `${BASE_URL}${accountInfo.service_site_logo}`);

  const handleGetApp = (e) => { openAppInstall(e); };
      ...
      <div className="max-w-[1440px] mx-auto px-4 md:px-8 relative z-10">
                ...
                <li key={link.label}>
                  {link.onClick ? (
                    <button
                      ...
                      className="text-left text-xs md:text-[13px] font-semibold text-gray-600 hover:text-[#1646d7] transition-all duration-200 flex items-center gap-2 group cursor-pointer hover:translate-x-1"
                    >
                      <FaChevronRight size={8} className="text-[#E49C16] group-hover:text-[#1646d7] transition-colors flex-shrink-0" />
                    ...
                    <Link
                      ...
                      className="text-xs md:text-[13px] font-semibold text-gray-600 hover:text-[#1646d7] transition-all duration-200 flex items-center gap-2 group cursor-pointer hover:translate-x-1"
                    >
                      <FaChevronRight size={8} className="text-[#E49C16] group-hover:text-[#1646d7] transition-colors flex-shrink-0" />
                      <span className="group-hover:underline underline-offset-2">{link.label}</span>
                    </Link>
        ...
        <div className="mt-3 grid grid-cols-1 lg:grid-cols-[1.35fr_auto] gap-3 lg:gap-5 items-center">
          ...
          <div className="flex flex-wrap items-center justify-start lg:justify-end gap-1.5">
            ...
            disabled={!isAppReady}
            className="inline-flex items-center gap-2 px-4 py-2 text-black font-black text-[10px] uppercase tracking-widest rounded-xl hover:-translate-y-1 transition-all duration-300 shadow-[0_10px_25px_-5px_rgba(34,197,94,0.35)] w-fit"
            style={{
