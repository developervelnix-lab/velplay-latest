...
const isSportsItem = (game) => {
  if (!game) return false;
  const provider = (game["Game Provider"] || game.provider || game.game_provider || "").toLowerCase().trim();
  const name = (game["Game Name"] || game.name || game.game_name || "").toLowerCase().trim();
  const category = (game["Game Type"] || game.category || game.game_category || "").toLowerCase().trim();
  ...
  if (sportsKeywords.some(sk => provider.includes(sk))) return true;
  if (provider === 'sport' || provider === 'sports') return true;
  if (['sport', 'sports', 'sportsbook', 'esport', 'e-sport', 'esports', 'e-sports'].includes(category)) return true;
  if (
    name.includes('saba sport') ||
    name.includes('saba sports') ||
    name.includes('lucky sports') ||
    name.includes('luck sports') ||
    name.includes('bti sport') ||
    name.includes('cmd368') ||
    name === 'sports' ||
    name === 'sport' ||
    name === 'sportsbook' ||
    name === 'esports' ||
...
const CasinoPage = () => {
  ...
  const spinPrizes = [
    { label: "₹500 Bonus", color: "#3b82f6", icon: "💰" },
    { label: "25 Free Spins", color: "#10b981", icon: "🎰" },
    { label: "₹1,000 Cash", color: "#f59e0b", icon: "💵" },
    { label: "10% Cashback", color: "#8b5cf6", icon: "💎" },
    { label: "₹2,500 High Roller", color: "#ec4899", icon: "👑" },
    { label: "50 Aviator Spins", color: "#ef4444", icon: "🚀" },
    { label: "2X Multiplier", color: "#06b6d4", icon: "⚡" },
    { label: "₹5,000 Mystery Box", color: "#eab308", icon: "🎁" },
  ];

  const handleSpinNow = () => {
    if (!accountInfo?.account_id || accountInfo.account_id === "guest") {
      setShowLogin(true);
      return;
    }
  ...
  const executeSpin = () => {
    ...
    const stopAngle = extraRounds + (spinPrizes.length - randomIndex) * sliceAngle - (sliceAngle / 2);

    setWheelRotation(prev => prev + stopAngle);

    setTimeout(() => {
      setIsSpinning(false);
      setSpinReward(spinPrizes[randomIndex]);
    }, 4000);
  };
      ...
      <div className="casino-shell">
        ...
        <div className="content">
          ...
          <div className="jp-bar">
            ...
            <button className="jp-btn" onClick={handleSpinNow}>SPIN NOW →</button>
          </div>
